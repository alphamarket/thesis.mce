classdef Q
    ...
    methods
        ...
        function [obj,action] =move(obj,Temperature,type)
           obj.length=obj.length+1;
           
           tmin=obj.minlen(obj.location,:);
           tmin(find(tmin==-1))=100;
           [~,tmini]= min(tmin');
           [~,tq]= max(obj.q(obj.location,:)');
           tempqqqq=obj.q(obj.location,:);
           if tmini~=tq && tmin(tmini)~=100
               tempqqqq(tmini)=tempqqqq(tq)+((tmin(tq)-tmin(tmini))*10*(tempqqqq(tq)-tempqqqq(tmini)));
           end
            if type==1
                action=selectActionBD(tempqqqq,Temperature);
            else
                action=selectActionBD(tempqqqq,Temperature);
                if isempty(action)
...
%                         end
%                     end
%             if sum(obj.minlen(obj.location)) == sum(abs(obj.minlen(obj.location)))
%                 action=selectActionBD(obj.minlen(obj.location,:),Temperature);
%             else
%                 action=selectActionBD(obj.q(obj.location,:),Temperature);
%             end
        end
